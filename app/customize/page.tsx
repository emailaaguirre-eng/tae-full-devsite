...
function CustomizeContent() {
  ...
  const getStudioSize = () => {
    if (selectedSize) {
      ...
      const width = parseInt(parts[0]) || 8;
      const height = parseInt(parts[1]) || 10;
      if (width > 0 && height > 0) {
        return { width, height, name: selectedSize };
      }
    }
    // Default fallback
  ...
  const handleVariantsComplete = () => {
    try {
      // Ensure we have uploaded images before proceeding
      if (uploadedImages.length === 0) {
        alert('Please upload an image first before proceeding to the design editor.');
        return;
      }
      setCurrentStep(4); // Move to Design Editor
    } catch (error) {
      console.error('Error in handleVariantsComplete:', error);
      alert('There was an error proceeding to the design editor. Please try again.');
    }
  };
  ...
  return (
    <>
      <Navbar />
      {/* Hero Section - shown only for steps 1 and 2 */}
      {(currentStep === 1 || currentStep === 2) && (
        <section 
          className="relative pt-24 pb-16 min-h-[60vh] flex items-center overflow-hidden"
          style={{
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div className="max-w-3xl">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 font-playfair">
                Create Your Artful Experience
              </h1>
              <p className="text-xl md:text-2xl text-white/90 mb-8 font-light">
                Transform your images into personalized art, cards, and gifts with ArtKey™ technology
              </p>
            </div>
          </div>
        </section>
      ...
      <div className="min-h-screen bg-brand-lightest pt-12 pb-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header - only show for steps 3+ */}
        {currentStep > 2 && (
        ...
        {showStudio && (
          <DesignEditor
            productType={
              productType === "postcard" || productType === "invitation" || productType === "announcement"
                ? "card"
                : productType === "print"
                ? "print"
                : "card"
                ...
                <button
                  onClick={() => {
                    try {
                      ...
                      const size = getStudioSize();
                      if (!size || size.width <= 0 || size.height <= 0) {
                        alert('Please select a valid product size.');
                        return;
                      }
                      setShowStudio(true);
                    } catch (error) {
                      console.error('Error opening design editor:', error);
                      alert('There was an error opening the design editor. Please try again.');
                    }
                  }}
                  disabled={uploadedImages.length === 0}
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-brand-lightest flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-brand-dark text-lg">Loading Customize...</p>
        </div>
      </div>
    }>
      <CustomizeContent />
