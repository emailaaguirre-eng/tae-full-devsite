...
import { getDb, saveDatabase, shopCategories, shopProducts, eq, desc, generateId } from '@/lib/db';
  ...
  try {
    ...
    const mapped = cats.map((c) => {
      ...
      return {
        ...
        name: c.name,
        icon: c.icon,
        printProvider: c.printProvider || 'printful',
        printfulProductId: c.printfulProductId,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await db.insert(shopCategories).values({
      ...
      requiresQrCode: body.requiresQrCode ? true : false,
