...
export default function AdminCoCreatorsPage() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      setError("Failed to load cocreators");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleToggleActive = async (item: CoCreator) => {
    setSaving(true);
    try {
  ...
  const handleDelete = async (item: CoCreator) => {
    if (!confirm(`Delete "${item.name}"? This cannot be undone.`)) return;
    setSaving(true);
    try {
      await fetch(`/api/admin/cocreators?id=${item.id}`, { method: "DELETE" });
      await fetchItems();
    } catch {
  ...
  const handleAddNew = async () => {
    ...
    try {
      const res = await fetch("/api/admin/cocreators", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: "New CoCreator", sortOrder: items.length }),
      });
      const data = await res.json();
      if (data.success && data.id) {
        window.location.href = `/b_d_admn_tae/cocreators/${data.id}`;
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
          <p className="text-gray-500 mb-4">No cocreators yet. The page will use static JSON data as fallback.</p>
          <button
            onClick={handleAddNew}
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Link
                        href={`/b_d_admn_tae/cocreators/${item.id}`}
