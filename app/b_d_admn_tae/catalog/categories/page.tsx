...
export default function CategoriesPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true);
    try {
      const response = await adminFetch(`/api/admin/store-categories/${editing.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleToggleActive = async (cat: Category) => {
    try {
      const response = await adminFetch(`/api/admin/store-categories/${cat.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (cat: Category) => {
    if (!confirm(`Delete category "${cat.name}"?\n\nThis will NOT delete the ${cat.productCount} products in it — move them first.`)) return;

    setDeletingId(cat.id);
    try {
      const response = await adminFetch(`/api/admin/store-categories/${cat.id}`, { method: 'DELETE' });
      const data = await response.json();
      if (data.success) {
