...
const TESTIMONIAL_DATA = [
  ...
  { name: "Katelyn", location: "Scottsdale, AZ realtor", mediaKey: "testimonials.9", defaultImage: "https://theartfulexperience.com/wp-content/uploads/2026/03/Katelyn-scaled.png", imageClassName: "object-contain", text: "The Artful Experience turned my closing gift into something unforgettable. My clients received a beautiful piece of art for their new home, and when they scanned it, a heartfelt message from me appeared. It’s meaningful, something they’ll hang prominently, and the kind of gift clients share with friends and neighbors - and hopefully future clients." },
  ...
  { name: "River", location: "Coach & Founder, Madinah\u2019s Living Farmacy", mediaKey: "testimonials.1", defaultImage: "https://theartfulexperience.com/wp-content/uploads/2026/03/River-scaled.jpeg", imageClassName: "object-contain", imageWrapperClassName: "relative aspect-[2/3] bg-gray-50 overflow-hidden rounded-t-2xl", text: "As a coach, I love how TheAE reinforces the power of imagery, affirmations, and interactive reflection. It transforms a single moment into a living reminder of the mindset where anything is possible. I would recommend theAE platform to any coach at any level." },
  ...
  { name: "Morgan", location: "Travel Influencer", mediaKey: "testimonials.2", defaultImage: "https://dredev.theartfulexperience.com/wp-content/uploads/2025/09/3125EEFB-C70A-4CF7-8DB3-1F4DB841E4B9-scaled.jpeg", imageClassName: "object-contain", text: "Rio de Janeiro holds a special place in my heart. I uploaded my photo from Ipanema Beach to TheAE, where it was artfully enhanced by Deanna Lankin. Now I can relive that moment anytime, and the ArtKey\u2122 interactivity, music, artwork, and inspiration brings the image to life." },
...
function TestimonialImage({ mediaKey, defaultImage, alt, className }: { mediaKey: string; defaultImage: string; alt: string; className?: string }) {
  const src = useSiteMedia(mediaKey, mediaUrl(defaultImage));
  if (!src) return null;
  return <Image src={src} alt={alt} fill className={className || "object-contain"} />;
}
