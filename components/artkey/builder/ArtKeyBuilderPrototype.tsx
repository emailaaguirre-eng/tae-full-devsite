...
export function ArtKeyBuilderPrototype({ visualMock = false }: ArtKeyBuilderPrototypeProps) {
  ...
  const [moduleOn, setModuleOn] = useState<Record<FeatureModuleId, boolean>>(
    () =>
      Object.fromEntries(
        FEATURE_MODULES.map((m) => [m.id, m.id === "favorites"])
            ...
            {tab === "features" && (
              <PanelShell title="Feature library">
                <p>
                  ...
                  settings. Rich blocks reuse the same content as the public portal mock
                  in <code className={styles.inlineCode}>PhoneModal.tsx</code> where titles
                  match (Sponsors, Stay Connected, Events, Supporter Updates).
                </p>
                <FeatureLibraryGrid
                  moduleOn={moduleOn}
            ...
            {tab === "save" && (
              <PanelShell title="Save">
                ...
                <div className={styles.publishBox}>
                  ...
                  <p className={styles.monoUrl}>
                    https://artkey.example/demo-portal-token
                  </p>
                </div>
              </PanelShell>
            )}
                ...
                <p className={styles.asideHint}>
                  ...
                  {visualMock ? (
                    ...
                    <>
                      ...
                      The phone shows module buttons (Video Gallery, Image Gallery, etc.)
                      with your font, shape, color, and style choices.
                    </>
                  ) : (
                    <>
                      {" "}
