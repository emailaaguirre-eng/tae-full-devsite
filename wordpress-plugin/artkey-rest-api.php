 ...
 * 
 * If product requires QR code (WooCommerce meta: _requires_qr_code = 'yes'),
 * the QR code will be generated by Next.js and stored in _artkey_qr_url
 */
function artkey_rest_save($request) {
    ...
    // Check if product requires QR code (if not already checked by Next.js)
    if ($product_id && !$requires_qr) {
        $product = wc_get_product($product_id);
        if ($product) {
            // Check for meta field indicating QR code requirement
            $requires_qr = $product->get_meta('_requires_qr_code') === 'yes' ||
                          $product->get_meta('_product_requires_qr') === 'yes' ||
                          $product->get_meta('requires_qr_code') === 'yes' ||
                          $product->get_meta('product_requires_qr_code') === 'yes';
        }
    }
    ...
    // If QR code URL is provided, store it
    if (isset($data['qr_code_url']) && $data['qr_code_url']) {
        $meta_input['_artkey_qr_url'] = $data['qr_code_url'];
    }
